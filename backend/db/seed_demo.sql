...
-- 37 заказов · 14 пользователей · документы · претензии · сообщения · аудит
-- ═══════════════════════════════════════════════════════════════════════════
...
-- Оценки свободных водителей (для каталога)
(4, 12, 5, 'Павел — пунктуальный водитель, тент в идеальном состоянии. Берём на постоянные рейсы.', DATE_SUB(NOW(),INTERVAL 20 DAY)),
(5, 12, 4, 'Хороший контакт, аккуратная погрузка. Рекомендую для коротких плеч.', DATE_SUB(NOW(),INTERVAL 12 DAY)),
(2, 13, 5, 'Андрей отлично работает с рефрижератором — температурный режим всегда выдержан.', DATE_SUB(NOW(),INTERVAL 16 DAY)),
(4, 13, 4, 'Надёжно, без срывов. Документы оформляет грамотно.', DATE_SUB(NOW(),INTERVAL 9 DAY)),
(3, 14, 5, 'Олег — опытный по ADR/МДП, спокойно проходит таможню. Очень доволен.', DATE_SUB(NOW(),INTERVAL 8 DAY)),
(5, 14, 4, 'Грамотный водитель, на связи. Небольшая задержка по погоде — не его вина.', DATE_SUB(NOW(),INTERVAL 6 DAY)),
-- Оценки перевозчиков (для каталога)
(4, 6,  5, 'ООО «АвтоЛогист» — надёжный парк, всегда есть свободные машины. Работаем годами.', DATE_SUB(NOW(),INTERVAL 33 DAY)),
(5, 7,  4, 'ИП Захаров — добросовестный перевозчик. Бортовые подаёт вовремя.', DATE_SUB(NOW(),INTERVAL 27 DAY)),
(2, 6,  5, 'Лучший партнёр по западному направлению. Без нареканий.', DATE_SUB(NOW(),INTERVAL 19 DAY));
...
-- Претензия 5: недостача груза (новая, на рассмотрении)
(16, 4, 9, 'cargo_damage', 'Новая', 750.00, 'EUR',
 'При приёмке в Берлине выявлена недостача 2 коробок относительно упаковочного листа. Получатель составил акт. Требуется проверка и возмещение.',
 NULL, NULL, NULL, DATE_SUB(NOW(),INTERVAL 2 DAY)),
-- Претензия 6: спор по простою (отклонена)
(13, 5, 10, 'delay', 'Отклонена', 400.00, 'BYN',
 'Заявлен простой ТС под выгрузкой более 6 часов сверх норматива. Перевозчик требует оплату простоя.',
 'По данным GPS и отметкам склада превышение норматива не подтверждено. Претензия отклонена.',
 1, DATE_SUB(NOW(),INTERVAL 6 DAY), DATE_SUB(NOW(),INTERVAL 9 DAY));
...
-- Свободные водители (12,13,14)
(12,'info',    'Профиль подтверждён',  'Ваш профиль верифицирован. Вы доступны для назначения заказов.',                        0, 'user', 12, DATE_SUB(NOW(),INTERVAL 4 DAY)),
(12,'info',    'Новые заявки рядом',   'Появились заявки из Минска в западном направлении. Ожидайте предложение диспетчера.',     0, NULL, NULL, DATE_SUB(NOW(),INTERVAL 1 DAY)),
(13,'info',    'Профиль подтверждён',  'Ваш профиль верифицирован. Рефрижераторные рейсы доступны для назначения.',              0, 'user', 13, DATE_SUB(NOW(),INTERVAL 3 DAY)),
(14,'info',    'Профиль подтверждён',  'Ваш профиль верифицирован. Доступны рейсы ADR/МДП.',                                     0, 'user', 14, DATE_SUB(NOW(),INTERVAL 2 DAY));
